...
class Config(object):
    ...
    gpu=True  #是否使用GPU
    nz=100  #噪声维度
    ngf=64  #生成器feature map 参数
    ndf=64  #判别器feature map 参数
    ...
    debug_file="/tmp/debuggan.txt" #存放该文件则进入debug模式
...
def generate(**kwargs):
    ...
    map_location=lambda storage, loc:storage
    netd.load_state_dict(t.load(opt.netd_path,map_loaction=map_loaction))
    netg.load_state_dict(t.load(opt.netg_path,map_loaction=map_loaction))
    netd.to(device)
    netg.to(device)
